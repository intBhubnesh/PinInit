...
const page = () => {
    ...
    const signUp = async (signUpData : SignUpData) => {
        try {
            const session = await authServices.signUp(signUpData)
                    ...
                    <form onSubmit={handleSubmit(signUp)}>
                        <div>
                            <div className="w-full ">
                                <InputField label="Name" type="text" {...register("name",{
                                    required : true,
                                    validate : (value : string) => /^[a-zA-Z]{2,50}(?:\s[a-zA-Z]{2,50})*$/.test(value) || "Enter a name with more than 2 charecter and no special charecter and symbols"
                                })}/>
                            </div>
                            <div className="w-full ">
                        ...
                        <div className="flex items-start justify-start gap-2 mt-6">
                            <div className="inline-flex w-5 items-center justify-center">
                                <input className="" type="checkbox" name="Term&Condition" id="Term&Condition" required/>
                            </div>
